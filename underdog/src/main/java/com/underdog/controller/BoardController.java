...
public class BoardController {
	...
	public String delProc(@RequestParam("bo_idx") int bo_idx, @RequestParam("bo_bbsid") int bo_bbsid) throws Exception {
		...
		if (bo_bbsid == 01) {
			jsp = "/board/info/bo_info_list?bo_bbsid=01";
			logger.info("공지사항으로 이동");
		} else if (bo_bbsid == 02) {
			jsp = "/board/free/bo_free_list?bo_bbsid=02";
			logger.info("자유게시판으로 이동");
		} else if (bo_bbsid == 03) {
			jsp = "/board/faq/bo_faq_list?bo_bbsid=03";
			logger.info("faq로 이동 이동");
		} else if (bo_bbsid == 04) {
			jsp = "/board/q&a/bo_q&a_list?bo_bbsid=04";
			logger.info("Q&A로 이동");
		}
